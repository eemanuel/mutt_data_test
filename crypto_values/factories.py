...
class DailyValuesFactoryBase(factory.django.DjangoModelFactory):
    usd_avg_value = factory.Faker(
        'pyfloat', positive=True, right_digits=CRYPTO_DECIMAL_SPACES, \
        min_value=10_000, max_value=50_000
    )
    usd_max_value = factory.LazyAttribute(
        lambda obj: round(uniform(obj.usd_avg_value + 1, obj.usd_avg_value + 10), 2)
    )
    usd_min_value = factory.LazyAttribute(
        lambda obj: round(uniform(obj.usd_avg_value - 10, obj.usd_avg_value - 1), 2)
    )

    ars_avg_value = factory.LazyAttribute(
        lambda obj: round(obj.usd_avg_value * ARS_VALUE, 2)
    )
